...
const KetuaTerpilih = () => {
    ...
    return (
        <div className="bg-[#4ccf24]">
            <div id="kandidat" className="flex flex-col items-center justify-center gap-8 bg-[#022960] p-4 pt-24 pb-8">
                <h1 className="text-5xl font-bold tracking-[0.1em] text-center font-newsweekly">HASIL PEMUNGUTAN SUARA</h1>
                            ...
                            return (
                                <div key={`button-lembaga-${item.id}`}>
                                    <label onClick={() => setLembaga(item.name)}>
                                        <div className="py-4 card btn-ghost btn h-full w-full cursor-pointer bg-[#023a89] shadow-xl transition duration-200">
                                            <div className="justify-center flex flex-col gap-2 items-center">
                                                <figure>
                                                    <img className="px-2 w-20" src={item.logo} alt={item.name} />
                                                </figure>
            ...
            <div id="kandidat" className="flex flex-col items-center justify-center gap-2 bg-[#022960] pt-8">
                <h1 className="text-2xl tracking-[0.1em] text-center">JUMLAH VOTE</h1>
            </div>
            <div className="flex flex-col items-center justify-centerpx-8 bg-[#022960]">
                        ...
                        candidate[0].map((k: any) => {
                            if (k.jumlah_vote === Math.max(...candidate[0].map((x: any) => x.jumlah_vote))) {
                                return (
                                    ...
                                    >
                                        <div className="rounded-full p-12 border-4 border-[#fb8500]">
                                            <h2 className="font-bold text-4xl text-[#fb8500]">{k.jumlah_vote}</h2>
                                        </div>
                                        <div
                                            key={`${lembaga}-${k.id}`}
                                        ...
                                        >
                                            ...
                                            <div className="flex flex-col items-center gap-2 text-center">
                                                ...
                                                <h2 className="pb-4 text-center text-4xl font-bold font-newsweekly">{k.name}</h2>
                                                <label
                                ...
                                return (
                                    <div
                                        ...
                                        className="card flex w-full flex-col items-center justify-start gap-4 px-4 py-8 "
                                    >
                                        <div className="rounded-full p-12 border-4 border-[#0098b2]">
                                            <h2 className="font-bold text-4xl text-[#00daff]">{k.jumlah_vote}</h2>
                                        </div>
                                        <div
                                            key={`${lembaga}-${k.id}`}
